...
{
    double eq1 = point.x * (first.y - second.y) + point.y * (second.x - first.x) + first.x * second.y - first.y * second.x;
    double eq2 = 1.0f / (third.x * (first.y - second.y) + third.y * (second.x - first.x) + first.x * second.y - first.y * second.x);
    return eq1 * eq2;
}

bool isBackfaceCulled(Vec3 *first, Vec3 *second, Vec3 *third, Vec3 v)
{
    Vec3 normalVector = crossProductVec3(*second - *first, *third - *first);
    Vec3 normalized = normalizeVec3(normalVector);

    double culling = dotProductVec3(normalized, v);

    return culling < 0.00005;
}
